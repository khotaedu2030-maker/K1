...
// يتحقق من رمز OTP، وعند صحته يُنهي جلسة الطالب فعليًا (لا مجرد إخفاء بصري) ويمسح الكوكي.
// العميل يرسل "code" فقط — لا نثق بأي "phone" قادم من الطلب؛ نعيد اشتقاق رقم جوال ولي الأمر
// من جلسة الطالب النشطة نفسها → الطفل → ولي الأمر → parents.phone، بنفس مسار request-otp تمامًا.
export async function POST(req: Request) {
  ...
  const { error } = await admin.auth.verifyOtp({ phone: parent.phone, token: code, type: "sms" });
